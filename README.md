...
docker run -d --name cc-proxy -p 3050:3050 -e PORT=3050 \
  -v "$PWD/data:/app/data" ghcr.io/maxeaglet/commandcode-proxy:release
```

> ⚠️ The `-v` mount is required. In this branch the key pool lives in `/app/data/keys.json` inside the container, so without a volume **recreating the container or updating the image wipes every key** — see [Data persistence](#data-persistence-important).
...
In this branch all key-pool state lives in a single JSON file (`/app/data/keys.json` inside the container). It sits in the **container writable layer** by default, so:
