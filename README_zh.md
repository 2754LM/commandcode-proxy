...
> ⚠️ 必须挂 `-v`。本分支的 Key 池只存在容器内的 `/app/data/keys.json`，不挂卷时**重建容器或更新镜像会把所有 Key 一起丢掉**，见[数据持久化](#数据持久化重要)。
...
本分支的 Key 池全部状态只存在一个 JSON 文件里（容器内路径 `/app/data/keys.json`）。它默认位于**容器可写层**，所以：
